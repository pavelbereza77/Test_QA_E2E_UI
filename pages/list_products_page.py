...
from pages.locators import ListProductPageLocators, BasketLocators, locator_button_add_basket, locator_name_product_link
...
class ListsProductPage(BasePage):
    ...
    def should_present_list_products(self):
        ...
        assert self.PRODUCT_NAME_LOCATOR in sale_list_product, f'Not {self.PRODUCT_NAME_LOCATOR} in list sale products'

    def should_present_button_product_add_to_basket(self):
        assert self.is_element_present(*locator_button_add_basket(
            name_product=self.PRODUCT_NAME_LOCATOR)), f'Not button add cart for {self.PRODUCT_NAME_LOCATOR}'

    def click_button_add_cart(self):
        self.browser.find_element(*locator_button_add_basket(self.PRODUCT_NAME_LOCATOR)).click()

    def click_link_product_test(self):
        self.browser.find_element(*locator_name_product_link(self.PRODUCT_NAME_LOCATOR)).click()

    def present_product_in_cart(self):
        list_product_in_cart = self.get_list_product_in_cart()
        assert self.PRODUCT_NAME_LOCATOR in list_product_in_cart, \
            f'Not {self.PRODUCT_NAME_LOCATOR} in basket'
